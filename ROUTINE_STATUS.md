...
### PR #15 — new this run — https://github.com/erBhushanPawar/lang-learner/pull/15 (branch `draft-lessons-2026-08-21`) — open
...
Note: b2-06 (Listening strategy) is officially marked "once audio is added" in `CURRICULUM.md`, but per the prior run's note it was drafted now as a strategy/vocab lesson (discourse markers for single-playback note-taking) to be paired with audio later.

With PR #15, the roadmap is now drafted (published or in an open PR) through **B2 #8**. All of A1, A2, B1, and B2 #1–8 are accounted for. Only B2 #9 (Speaking prep) and B2 #10 (Full-length timed mock exam) remain undrafted in the whole curriculum.
...
First check whether the CI issue has been resolved and whether any of the open-PR content (b1-02 through b2-08, plus PR #6/#8's merged-but-unpublished a2-07/08/09 and a2-13/14/b1-01) has gone live. Do not re-draft anything already covered by an open PR or a merged-but-unpublished draft file in `data/drafts/` on `main`.

Once B2 #1–8 are live (or drafted with nothing new to add), the last two curriculum items are:

1. **B2 #9 — Speaking prep** — describing an experience, summarizing a text, discussion phrases. *Note: curriculum marks this "once speaking practice is added" — same situation as b2-06; likely draftable now as a phrases/strategy lesson.*
2. **B2 #10 — Full-length timed mock exam** — leans on the already-built exam-prep track; may not fit the standard vocab/grammarNote/check lesson schema as cleanly as prior lessons, worth flagging for human judgment on scope before drafting.

After that, the entire CURRICULUM.md roadmap will be drafted end-to-end (modulo the unresolved A1 #1 pronunciation question and the still-unpublished CI backlog above) — future runs should shift focus to checking whether open PRs have been merged/published rather than finding new content to draft.

---
Last run: 2026-08-21. Confirmed via Supabase: still a1-01–11, a2-01–06, a2-10–12 published (20 lessons) — no change since 2026-08-15. Re-verified GitHub Actions: no `publish-drafts.yml` run since 2026-08-14, and PRs #9–#14 all still open (unchanged from prior run). Drafted b2-06 (listening strategy), b2-07 (formal letter/email writing), b2-08 (article/essay writing) as new PR #15 — completes B2 #6–8, leaving only B2 #9–10 undrafted in the entire curriculum.
